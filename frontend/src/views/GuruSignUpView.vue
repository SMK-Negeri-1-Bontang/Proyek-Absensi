...
<template>
  <div
    :class="[
      'flex items-center justify-center sm:py-[150px] min-h-screen',
      isSmallScreen
        ? 'bg-gray-800'
        : `bg-[url('@/components/images/kde_mountain.jpg')] bg-cover`,
    ]"
  >
    <div
      class="w-full max-w-full bg-gray-800 p-8 pt-16 rounded-lg sm:shadow-lg sm:max-w-md"
    >
      ...
      <form @submit.prevent="signup">
        <div class="mb-4">
          <label class="block text-gray-400 text-sm font-bold mb-2" for="nama"
            >Nama</label
          >
          <input
            ...
            v-model="form.nama.content"
          />
          <p v-if="form.nama.error" class="text-red-500 text-sm mt-2">
            {{ form.nama.error }}
          </p>
        </div>

        <div class="mb-4">
          <label class="block text-gray-400 text-sm font-bold mb-2" for="nip"
            >NIP</label
          >
          <input
            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-100"
            ...
            v-model="form.nip.content"
          />
          <p v-if="form.nip.error" class="text-red-500 text-sm mt-2">
            {{ form.nip.error }}
          </p>
        </div>
        ...
        <div class="mb-4">
          ...
          />
          <p v-if="form.password.error" class="text-red-500 text-sm mt-2">
            {{ form.password.error }}
          </p>
        </div>
        ...
        <div class="mb-2">
          ...
          />
          <p
            v-if="form.confirmPassword.error"
